...
public class OVRCameraRig : MonoBehaviour
{
		...
		if (AR_is_Active){									// If AR world is on, turn off VRLayer = 11
			foreach (Camera c in cameras){
				...
				if (c.name == "LeftEyeAnchor"){
					c.cullingMask = ~(1<<9) & ~(1<<11) & ~(1<<17) & ~(1<<19);				
				}
				else {
					c.cullingMask = ~(1<<8) & ~(1<<11) & ~(1<<17) & ~(1<<18);
				}
			}
			...
			foreach (Camera c in cameras){
				//Debug.Log ("Camera Parent: "+c.name);
				if (c.name == "LeftEyeAnchor"){
					c.cullingMask = ~(1<<9) & ~(1<<10) & ~(1<<17) & ~(1<<19);				
				}
				else {
					c.cullingMask = ~(1<<8) & ~(1<<10) & ~(1<<17) & ~(1<<18);
				}
	...
	private void UpdateAnchors()
	{
		...
		rightEyeAnchor.localPosition = new Vector3 (rightEye.position.x, rightEye.position.y, rightEye.position.z) * 10.0f;
